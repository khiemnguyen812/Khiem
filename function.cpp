...
void MakeSnake(Snake ran) {
	int i;
	ran.bodyPattern.resize(8);
	ran.bodyPattern = { '2' ,'1','1','2','7','7','2','6'};
	ran.body.resize(8);
	for (i = 0; i < 8; i++) {
		ran.body[i].X = 89 - 8 + i;
		ran.body[i].Y = 1;
...
void Dichuyenran(Snake snake,COORD temp) {
	int i;
	for (i = snake.body.size(); i > 0 ; i--) {
		snake.body[i].X = snake.body[i - 1].X;
		snake.body[i].Y = snake.body[i - 1].Y;
	}
	snake.body[0].X = temp.X;
	snake.body[0].Y = temp.Y;
}

void MakeSnakeMove(Snake snake) {
	COORD temp;
	temp = snake.body[0];
	int check = 1;
	// 1: dch sang trai
